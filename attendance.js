...
function initAttendance(){
  ...
  document.getElementById("markAllAbsentBtn").addEventListener("click",()=>{attServants.forEach(s=>rowStatus[s.id]="absent");renderAttendance()});
...
function loadExisting(){
  ...
  attServants.forEach(s=>rowStatus[s.id]=records.find(r=>r.servantId===s.id)?.status||"absent");
  renderAttendance();
}
function renderAttendance(){
  ...
  body.innerHTML=list.length?list.map(s=>`<tr><td><strong>${escapeHTML(s.name)}</strong></td><td><button class="status-btn present ${rowStatus[s.id]==="present"?"active":""}" onclick="setStatus('${s.id}','present')">حاضر</button></td><td><button class="status-btn absent ${rowStatus[s.id]==="absent"?"active":""}" onclick="setStatus('${s.id}','absent')">غياب</button></td><td><button class="status-btn late ${rowStatus[s.id]==="late"?"active":""}" onclick="setStatus('${s.id}','late')">تأخير</button></td></tr>`).join(""):`<tr><td colspan="4"><div class="empty-state">لا يوجد خدام</div></td></tr>`;
  updateAttStats();
}
